        ...
        <div class="px-6 py-4">
            ...
            <form action="{{ route('productos.update', $producto->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="nombre_producto" type="text" name="nombre_producto" value="{{ $producto->nombre_producto }}">
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="descripcion_producto" name="descripcion_producto" rows="3">{{ $producto->descripcion_producto }}</textarea>
                </div>
                    ...
                    <div class="relative">
                        <select class="block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline" id="category_id" name="category_id">
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" {{ $producto->category_id == $category->id ? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mb-6">
                    ...
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="fecha_producto" type="date" name="fecha_producto" value="{{ $producto->fecha_producto->format('Y-m-d') }}">
                </div>
